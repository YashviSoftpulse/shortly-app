...
function useSparkBar({
  ...
  const yScale = d3Scale.scaleLinear().range(calculateRange(defaultData.data, height)).domain([Math.min(...filteredData, targetValue, 0), Math.max(...filteredData, targetValue, 0)]);
  const targetLineYPosition = yScale(targetValue);
  const xScale = d3Scale.scaleBand().range([offsetLeft, width - offsetRight]).paddingInner(BAR_PADDING).domain(defaultData.data.map((_, index) => index.toString()));
  const barWidth = react.useMemo(() => xScale.bandwidth(), [xScale]);
  const strokeDashoffset = offsetLeft == null ? -(STROKE_WIDTH / 2) : -(STROKE_WIDTH / 2) - offsetLeft;
  const getBarHeight = react.useCallback(value => {
  ...
  const color = getGradientFromColor.getGradientFromColor(colorToUse);
  return {
    dataWithIndex,
    color,
    strokeDashoffset,
...
exports.useSparkBar = useSparkBar;
