'use strict';

Object.defineProperty(exports, '__esModule', { value: true });

var React = require('react');
var d3Scale = require('d3-scale');
var polarisVizCore = require('@shopify/polaris-viz-core');
var index = require('../ChartElements/index.js');
var constants = require('../../constants.js');
var jsxRuntime = require('react/jsx-runtime');
var useReducedLabelIndexes = require('../../hooks/useReducedLabelIndexes.js');
var FunnelSegment = require('./components/FunnelSegment.js');
var FunnelChartXAxisLabels = require('./components/FunnelChartXAxisLabels.js');

const X_LABEL_OFFSET = 16;
const NEGATIVE_LABEL_OFFSET = -4;
function Chart({
  data,
  dimensions,
  xAxisOptions,
  yAxisOptions,
  labelHelpers
}) {
  const {
    theme
  } = polarisVizCore.useChartContext();
  const selectedTheme = polarisVizCore.useTheme();
  const [xAxisHeight, setXAxisHeight] = React.useState(polarisVizCore.LINE_HEIGHT);
  const dataSeries = data[0].data;
  const colorOverride = data[0].color;
  const [maskRef, setMaskRef] = React.useState(null);
  const xValues = dataSeries.map(({
    key
  }) => key);
  const yValues = dataSeries.map(({
    value
  }) => value);
  const {
    width,
    height
  } = dimensions || {
    width: 0,
    height: 0
  };
  const labels = React.useMemo(() => dataSeries.map(({
    key
  }) => xAxisOptions.labelFormatter(key)), [dataSeries, xAxisOptions]);
  const xScale = d3Scale.scaleBand().domain(xValues).range([0, width]).paddingInner(0.5);
  const labelXScale = d3Scale.scaleBand().range([0, width]).paddingInner(0.5).paddingOuter(0).domain(labels.map((_, index) => index.toString()));
  const drawableHeight = height - xAxisHeight - X_LABEL_OFFSET;
  const yScale = d3Scale.scaleLinear().range([0, drawableHeight - constants.BAR_CONTAINER_TEXT_HEIGHT]).domain([0, Math.max(...yValues)]);
  const barWidth = xScale.bandwidth();
  const getBarHeight = React.useCallback(rawValue => {
    const rawHeight = Math.abs(yScale(rawValue) - yScale(0));
    const needsMinHeight = rawHeight < polarisVizCore.MIN_BAR_HEIGHT && rawHeight !== 0;
    return needsMinHeight ? polarisVizCore.MIN_BAR_HEIGHT : rawHeight;
  }, [yScale]);
  const reducedLabelIndexes = useReducedLabelIndexes.useReducedLabelIndexes({
    dataLength: data[0] ? data[0].data.length : 0
  });
  const color = colorOverride || selectedTheme.seriesColors.single;
  const barsGradient = polarisVizCore.getGradientFromColor(color);
  const averageColor = polarisVizCore.getAverageColor(barsGradient[0].color, barsGradient.length > 1 ? barsGradient[1].color : barsGradient[0].color);
  const connectorGradientId = React.useMemo(() => polarisVizCore.uniqueId('connector-gradient'), []);
  const connectorGradient = [{
    color: polarisVizCore.changeColorOpacity(averageColor, 0.2),
    offset: 0
  }, {
    color: polarisVizCore.changeColorOpacity(averageColor, 0),
    offset: 100
  }];
  const gradientId = React.useMemo(() => polarisVizCore.uniqueId('gradient'), []);
  const maskId = React.useMemo(() => polarisVizCore.uniqueId('mask'), []);

  const handlePercentLabelFormatter = percentCalculation => {
    const percentRounded = Math.round(percentCalculation);

    if (percentCalculation) {
      return `${yAxisOptions.labelFormatter(percentRounded)}%`;
    } else {
      return '';
    }
  };

  return /*#__PURE__*/jsxRuntime.jsxs(index.ChartElements.Svg, {
    height: height,
    width: width,
    children: [/*#__PURE__*/jsxRuntime.jsx(polarisVizCore.LinearGradientWithStops, {
      gradient: connectorGradient,
      id: connectorGradientId,
      x1: "0%",
      x2: "0%",
      y1: "100%",
      y2: "0%"
    }), /*#__PURE__*/jsxRuntime.jsx(polarisVizCore.LinearGradientWithStops, {
      gradient: barsGradient,
      id: `${gradientId}`
    }), /*#__PURE__*/jsxRuntime.jsx("mask", {
      ref: setMaskRef,
      id: `${maskId}-${theme}-grad`
    }), dataSeries.map((dataPoint, index) => {
      const nextPoint = dataSeries[index + 1];
      const xPosition = xScale(dataPoint.key);
      const x = xPosition == null ? 0 : xPosition;
      const nextBarHeight = getBarHeight((nextPoint === null || nextPoint === void 0 ? void 0 : nextPoint.value) || 0);
      const yAxisValue = dataPoint.value;
      const percentCalculation = nextPoint !== null && nextPoint !== void 0 && nextPoint.value && yAxisValue ? nextPoint.value / yAxisValue * 100 : 0;
      const barHeight = getBarHeight(dataPoint.value || 0);
      const percentLabel = handlePercentLabelFormatter(percentCalculation);
      const formattedYValue = yAxisOptions.labelFormatter(yAxisValue);
      const labelHelper = labelHelpers === null || labelHelpers === void 0 ? void 0 : labelHelpers.find(helper => helper.key === dataPoint.key);
      return /*#__PURE__*/jsxRuntime.jsx(React.Fragment, {
        children: maskRef && /*#__PURE__*/jsxRuntime.jsx("g", {
          role: "listitem",
          children: /*#__PURE__*/jsxRuntime.jsx(FunnelSegment.FunnelSegment, {
            percentLabel: percentLabel,
            labelHelper: labelHelper && labelHelper.value,
            formattedYValue: formattedYValue,
            isLast: index === dataSeries.length - 1,
            connector: {
              height: drawableHeight,
              startX: x + barWidth,
              startY: drawableHeight - barHeight,
              nextX: xScale(nextPoint === null || nextPoint === void 0 ? void 0 : nextPoint.key),
              nextY: drawableHeight - nextBarHeight,
              nextPoint,
              fill: `url(#${connectorGradientId})`
            },
            ariaLabel: `${xAxisOptions.labelFormatter(dataPoint.key)}: ${yAxisOptions.labelFormatter(dataPoint.value)}`,
            barWidth: barWidth,
            barHeight: barHeight,
            color: polarisVizCore.MASK_HIGHLIGHT_COLOR,
            x: x,
            portalTo: maskRef,
            index: index,
            drawableHeight: drawableHeight
          })
        }, dataPoint.key)
      }, dataPoint.key);
    }), /*#__PURE__*/jsxRuntime.jsx("g", {
      "aria-hidden": "true",
      children: /*#__PURE__*/jsxRuntime.jsx(FunnelChartXAxisLabels.FunnelChartXAxisLabels, {
        allowLineWrap: xAxisOptions.allowLineWrap,
        chartX: barWidth / NEGATIVE_LABEL_OFFSET,
        chartY: drawableHeight + X_LABEL_OFFSET,
        labels: labels,
        labelWidth: barWidth + barWidth / 2,
        onHeightChange: setXAxisHeight,
        reducedLabelIndexes: reducedLabelIndexes,
        xScale: labelXScale
      })
    }), /*#__PURE__*/jsxRuntime.jsx("rect", {
      mask: `url(#${maskId}-${theme}-grad)`,
      x: 0,
      y: 0,
      width: width,
      height: drawableHeight,
      fill: `url(#${gradientId})`,
      style: {
        pointerEvents: 'none'
      }
    })]
  });
}

exports.Chart = Chart;
