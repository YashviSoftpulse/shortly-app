...
function getNegativeOffset(props) {
  ...
  return {
    x: flippedX - tooltipDimensions.width - utilities.TOOLTIP_MARGIN,
    y: currentY + polarisVizCore.HORIZONTAL_GROUP_LABEL_HEIGHT + yOffset
  };
}
...
function getPositiveOffset(props) {
  ...
  if (isOutside.top && !isOutside.right) {
    return {
      x: currentX + utilities.TOOLTIP_MARGIN,
      y: 0
    };
  }
  ...
  if (isOutside.right) {
    ...
    const y = currentY - tooltipDimensions.height + polarisVizCore.HORIZONTAL_GROUP_LABEL_HEIGHT - utilities.TOOLTIP_MARGIN;

    if (y < 0) {
      return {
        x,
        y: bandwidth + polarisVizCore.HORIZONTAL_GROUP_LABEL_HEIGHT + utilities.TOOLTIP_MARGIN
      };
    }
  ...
  if (isOutside.bottom) {
    return {
      x: currentX + utilities.TOOLTIP_MARGIN,
      y: chartBounds.height - tooltipDimensions.height - polarisVizCore.HORIZONTAL_GROUP_LABEL_HEIGHT
    };
  }
...
function isOutsideBounds({
  ...
  const right = x + utilities.TOOLTIP_MARGIN + tooltipDimensions.width;
...
exports.getAlteredHorizontalBarPosition = getAlteredHorizontalBarPosition;
